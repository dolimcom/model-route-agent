...
手动模式创建 `PENDING` 提案，批准前不会修改文件系统。全权模式立即执行，但仍会创建审计记录。回滚是尽力而为的，并且只覆盖已记录的单次操作；执行后发生的外部编辑可能与回滚冲突。
...
Manual mode creates a `PENDING` proposal and does not touch the filesystem before approval. Full-access mode executes immediately but still creates an audit row. Rollback is best-effort and only covers the recorded operation; external edits made after execution can conflict with rollback.
